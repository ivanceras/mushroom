...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		/// A content is posted
		ContentPosted(Vec<u8>, T::AccountId),
	}
	...
	pub type Post<T> = StorageValue<_, Vec<u8>>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_name(origin: OriginFor<T>, mut name: Vec<u8>) -> DispatchResult {
		...
		/// An example call which stores a simple Data content
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn post_content(origin: OriginFor<T>, content: Vec<u8>) -> DispatchResult {
			...
			// Update storage.
			<Post<T>>::put(content.clone());

			// Emit an event.
			Self::deposit_event(Event::ContentPosted(content, who));
