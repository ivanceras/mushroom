...
fn main() {
    ...
    let result: Option<Vec<u8>> = api.get_storage_value("Nicks", "Post", None).unwrap();
    let post = result.map(|r| std::str::from_utf8(&r).unwrap().to_string());
    println!("[+] post : {:?}", post);

    let alice = AccountKeyring::Alice;

    let result: Option<Vec<u8>> = api
        .get_storage_map("Nicks", "NameOf", account, None)
        .unwrap();
    let nick_name = result.map(|r| std::str::from_utf8(&r).unwrap().to_string());
