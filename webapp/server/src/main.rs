...
fn fake_api_call(name: String) -> Data {
    let metadata = node_events::get_metadata();
    Data {
        length: name.len(),
...
async fn main() {
    ...
    let named = warp::path!(String).map(render_page);

    // Render paths that don't include a name with a default
    let root = warp::path::end().map(move || render_page(String::from("Ferris")));
...
/*
fn main() {
    node_events::show_metadata();
    node_events::get_storage();
    node_events::listen_to_events();
}
*/
